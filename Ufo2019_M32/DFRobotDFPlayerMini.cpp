...
//#ifdef _DEBUG
  ...
  //Serial.println();
//#endif
  sc16is_uart_write_buf(_sending, DFPLAYER_SEND_LENGTH);
...
bool DFRobotDFPlayerMini::available(){
  while (sc16is_available()) {
    _delay_ms(0);
    if (_receivedIndex == 0) {
      _received[Stack_Header] = sc16is_uart_read();
//#ifdef _DEBUG
...
//#endif
      ...
      _received[_receivedIndex] = sc16is_uart_read();
//#ifdef _DEBUG
      //Serial.print(_received[_receivedIndex],HEX);
      //Serial.print(F(" "));
//#endif
